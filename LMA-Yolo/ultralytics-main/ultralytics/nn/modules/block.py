...
class MyC2f(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        # self.c2_1 = C2f(c1,c2,n=1)
        # self.c2_2 = C2f(c1,c2,n=2)

        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(ConvBlock(self.c) for _ in range(n)))

        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc1 = nn.Conv2d(self.c, self.c, 1, 1, 0, bias=True)
        self.fc2 = nn.Conv2d(self.c, self.c, 1, 1, 0, bias=True)
        self.act = nn.Sigmoid()

    def forward(self, x):
        a, b = self.cv1(x).chunk(2, 1)
        a = a * self.act(self.fc1(self.pool(a)))
        b = b * self.act(self.fc1(self.pool(b)))
        return self.cv2(torch.cat((self.m(a), b), 1))
...
class MyModule(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c3_1 = C2f(c1,c2,n=1)
        self.c3_2 = C2f(c1,c2,n=2)
    ...
    def forward(self, x):
        ...
        x1 = weight[0] * self.c3_1(x)+weight[1] * self.c3_1(x)
        x1 = self.sigmoid(x)*x1 +x
        return x1
...
class C2ff(nn.Module):
    ...
    def forward(self, x):
        y = list(self.cv1(self.bd(x)).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))

    def forward_split(self, x):
        y = list(self.cv1(self.bd(x)).split((self.c, self.c), 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
...
class GMC(nn.Module):
    def __init__(self, in_channels, num_heads=4, group_split=None, kernel_sizes=None, window_size=4, 
        ...
        elif group_split is None:
            group_split = [num_heads // 2, num_heads // 2]
        ...
        # 减少通道数，然后再融合
        self.reduce_channels = nn.Conv2d(in_channels * 2, in_channels, kernel_size=1, bias=False)
        self.fusion = nn.Conv2d(in_channels, in_channels, kernel_size=1, bias=False, groups=in_channels)
        
    def forward(self, x):
        gam_out = self.gam(x)
        eff_out = self.efficient_attn(x)
        
        fused = torch.cat([gam_out, eff_out], dim=1)
        fused = self.reduce_channels(fused)
...
class C2f_DWConv(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = DWConv(c1, 2 * self.c)
        self.cv2 = DWConv((2 + n) * self.c, c2, 1)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
